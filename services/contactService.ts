...
export const contactValidationSchema = z.object({
  name: z
    ...
    .regex(/^[a-zA-Z\sñÑáéíóúÁÉÍÓÚüÜ]*$/, {
      message: "Name can only contain letters and spaces.",
    }),
  email: z.email({ message: "Please enter a valid email address." }),
  target_channel: z.enum(["telegram", "slack", "email", "discord"], {
    message: "Invalid target channel.",
  }),
  message: z
    .string()
...
export const executeFormSubmission = async ({
  ...
  if (!apiUrl) {
    setStatus("error");
    toast.error("Configuration Error: API Gateway URL is missing.", {});
    return;
  }
